...
%% LK. zmeneno
\ref{sec:gdal}] textový soubor ve formátu \zk{VFK}. Driver při otevření
%% ML: SQL -> SQLite
%% LK: opraveno
souboru vytvoří SQLite\footnote{Relační databázový systém} databázi, do které se uloží načtená data. \zk{VFK} soubor již není
%% ML: SQL -> SQLite
%% LK: opraveno
dále využíván, knihovna místo toho přistupuje k vytvořené SQLite
databázi. Důvodem je fakt, že VFK Driver[\ref{subsec:gdal_vfk}] sice čte data také z databáze, ale není schopen načíst později přidané bloky parcel a budov. Bloky chybí v podkladovém \zk{VFK} souboru, na základě kterého VFK driver pozná datové vrstvy v databázi. Následně je zkontrolována verze knihovny
...
%% LK: doplneno
Pro správné fungování načítání dat z databáze(pokud je verze knihovny GDAL[\ref{sec:gdal}] nižší než 2.2) je nezbytné vložit do databáze
...
nastane chyba. To se může stát není-li připojen \zk{VFK},
...
%% LK: upraveno
Základem integrace knihovny do zásuvného modulu je
