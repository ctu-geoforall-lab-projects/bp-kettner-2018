...
%% Abstrakt
\abstrakt{Tato bakalářská práce je zaměřena na rozšíření již existujícího zásuvného modulu pro práci s katastrálními daty o možnost využití i nekomerčních, volně dostupných (nezpoplatněných) dat ve formátu VFK. Bude se jednat o sestavení bloků PAR a BUD (parcel a budov), které lze z dostupných geometrických a popisných informací sestavit.
Zásuvný modul bude vyvíjen pro prostředí open source nástroje QGIS v programovacím jazyce Python.}
{This bachelor thesis is focused on development of already existing plugin for working with cadastral data in VFK format. The project will enable the use of non-commercial, for free available data in VFK format. That means creat new blocks PAR and BUD (parcels and buildings) from geometric and discription information which are included. The plugin will be developed in open source project QGIS in programming language Python.}
